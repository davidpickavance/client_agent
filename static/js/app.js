...
function showVideoStreams(converstion) {

	$('#local-video').empty(); // remove the image placeholders
	$('#remote-video').empty();

	converstion.localMedia.attach('#local-video'); // show local video
	$('#local-video-label').html(myEndpoint.address); // show local address

	conversation.on('particpantConnected', function(participant) {
...
document.getElementById('hangup').onclick = function() {
	...
	for (conversation of myEndpoint.conversations) {
...
document.getElementById('preview').onclick = function() {
	...
	Twilio.getUserMedia().then(
		...
		function(error) {
			console.eror("UInable to access local media", error);
